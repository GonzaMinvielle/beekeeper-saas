...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={pending}
      ...
      {pending ? 'Guardando...' : 'Registrar tratamiento'}
    </button>
  )
}

export default function TreatmentForm({ hives }: { hives: HiveWithApiary[] }) {
  const [state, action] = useFormState(createTreatment, {})
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Nuevo tratamiento</h1>
      </div>
        ...
        <form action={action} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="product_name" type="text" required placeholder="Ej: Oxálico, Amitraz..."
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                           focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Dosis</label>
              <input name="dose" type="text" placeholder="Ej: 50ml, 2 tiras"
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                           focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea name="notes" rows={3} placeholder="Observaciones del tratamiento..."
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
